...
from WaterFrameBuffers import WaterFrameBuffers

from MyTrackball import *
from Utils import *
import random as rd
...
class VertexArray:
    def recompute_atts(self, shader, attributes, usage=GL.GL_STATIC_DRAW):
        for name, data in attributes.items():
            ...
            if loc >= 0:
                ...
                self.nb_primitives, self.size = data.shape
                GL.glEnableVertexAttribArray(loc)
                GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self.buffers[name])
                GL.glBufferData(GL.GL_ARRAY_BUFFER, data, usage)
                GL.glVertexAttribPointer(loc, self.size, GL.GL_FLOAT, False, 0, None)
    ...
    def __init__(self, shader, attributes, index=None, usage=GL.GL_STATIC_DRAW):
        ...
        self.nb_primitives, self.size = 0, 0

        # load buffer per vertex attribute (in list with index = shader layout)
        self.recompute_atts(shader, attributes, usage)

        # optionally create and upload an index buffer for this object
        self.draw_command = GL.glDrawArrays
        self.arguments = (0, self.nb_primitives)
    ...
    def execute(self, primitive, attributes=None):
        ...
        for name, data in attributes.items():
            # test name existence : in case no name var in shader or optimized out
...
def load(file, shader, tex_file=None, delete_tex=False, do_print=True, pre_tex=None, **params):
    ...
    for mat in scene.mMaterials:
        if tex_file:
            ...
            tfile = None
        if pre_tex:
            mat.properties['diffuse_map'] = pre_tex
        elif Texture is not None and tfile:
...
class Viewer(Node):
    ...
    def set_water_x(self, x0, x1):
        self.water_x = (x0, x1)

    def is_x_in_water(self, x):
        return (x >= self.water_x[0]) and (x < self.water_x[1])

    def get_rd_x_outside_of_water(self, x0, x1):
        x = rd.randint(x0, x1)
        while self.is_x_in_water(x):
            x = rd.randint(x0, x1)
        return x

    def run(self, player, water, ground=None):
        """ Main render loop for this OpenGL window """

        from Tumbleweed import Tumbleweed  # ouai c'est nul Python

        self.player = player
        self.trackball.set_player(self.player)
        self.light = ground.get_light()
        self.ground = ground
        fbos = WaterFrameBuffers()
        water.set_fbos(fbos)
        ...
        while not glfw.window_should_close(self.win):
            ...
            distance = 2 * (self.trackball.get_position()[2] - water_height)
            self.trackball.set_additional_z(-distance)
            self.trackball.invert_pitch()
            self.draw(view=self.trackball.view_matrix(),
                      projection=self.trackball.projection_matrix(win_size),
                      model=identity(),
                      w_camera_position=self.trackball.get_position(),
                      plane=np.array([0, 0, 1, water_height]),
                      light_dir=self.light
                      )
            self.trackball.unset_additional_z()
            self.trackball.invert_pitch()
            ...
            # GL.glViewport(0, 1080 - 600, 600, 600)
            # distance = 2 * (self.trackball.get_position()[2] - water_height)
            # self.trackball.set_additional_z(-distance)
            # self.trackball.invert_pitch()
            # self.draw(view=self.trackball.view_matrix(),
            ...
            # GL.glBindTexture(GL.GL_TEXTURE_2D, 0)
            # GL.glBindFramebuffer(GL.GL_FRAMEBUFFER, 0)
            # GL.glViewport(1920 - 600, 1080 - 600, 600, 600)
            # self.draw(view=self.trackball.view_matrix(),
            #           projection=self.trackball.projection_matrix(win_size),
            #           model=identity(),
            #           w_camera_position=self.trackball.get_position(),
            ...
            self.player.move(self.win)
            if rd.random() < 0.06:
                x = self.get_rd_x_outside_of_water(2, 100)
                self.add(Tumbleweed.generate(x, 2, 72, 0.86, self))
    ...
    def on_key(self, _win, key, _scancode, action, _mods):
        if action == glfw.PRESS or action == glfw.REPEAT:
            ...
            #     glfw.set_time(0.0)
            if key == glfw.KEY_F:
                self.player.invert_make_smoke()
            if key == glfw.KEY_B:
                self.ground.theta = self.ground.theta + 0.001
            if key == glfw.KEY_N:
                self.ground.theta = self.ground.theta - 0.001
            if key == glfw.KEY_H:
                self.ground.theta = 0
            if key == glfw.KEY_J:
                self.ground.theta = 0.01
    ...
    def on_mouse_move(self, win, xpos, ypos):
        ...
        if glfw.get_mouse_button(win, glfw.MOUSE_BUTTON_LEFT):
            self.trackball.on_mouse_move(xpos - self.mouse[0], self.mouse[1] - ypos)

        self.mouse = (xpos, ypos)

    def on_scroll(self, win, _deltax, deltay):
        self.trackball.on_mouse_scroll(deltay)
